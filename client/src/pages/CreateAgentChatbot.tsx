        ...
        <div className="max-w-7xl mx-auto">
            ...
            <div className="flex gap-6">
              ...
              <div className="w-64 space-y-2">
                ...
                <Button
                  variant={activeTab === "web-search" ? "default" : "ghost"}
                  className="w-full justify-start"
                      ...
                      <div className="space-y-6">
                        ...
                        <Card>
                          <CardHeader>
                            ...
                            <CardDescription>
                              Configure web search settings and whitelisted websites for your agent
                            </CardDescription>
                          </CardHeader>
                          <CardContent className="space-y-6">
                            ...
                            <div className="bg-blue-50 rounded-lg p-4 space-y-4">
                              <div className="flex items-center justify-between">
                                <div>
                                  <Label className="text-base font-medium">Enable Web Search</Label>
                                  <p className="text-sm text-gray-600">
                                    Allow this agent to search the web for real-time information
                                  </p>
                                </div>
                                <Switch
                                  checked={existingAgent?.webSearchConfig?.enabled || false}
                                  onCheckedChange={async (enabled) => {
                                    try {
                                      const response = await fetch(`/api/agent-chatbots/${editAgentId}/web-search-config`, {
                                        method: "PUT",
                                        headers: { "Content-Type": "application/json" },
                                        body: JSON.stringify({
                                          enabled,
                                          triggerKeywords: existingAgent?.webSearchConfig?.triggerKeywords || [],
                                          maxResults: existingAgent?.webSearchConfig?.maxResults || 5,
                                          requireWhitelist: existingAgent?.webSearchConfig?.requireWhitelist !== false
                                        }),
                                      });

                                      if (!response.ok) {
                                        throw new Error("Failed to update web search configuration");
                                      }

                                      toast({
                                        title: "Configuration Updated",
                                        description: `Web search ${enabled ? 'enabled' : 'disabled'} successfully`,
                                      });

                                      // Refetch agent data to update UI
                                      queryClient.invalidateQueries({ queryKey: [`/api/agent-chatbots/${editAgentId}`] });
                                    } catch (error) {
                                      toast({
                                        title: "Error",
                                        description: "Failed to update web search configuration",
                                        variant: "destructive",
                                      });
                                    }
                                  }}
                              ...
                              {existingAgent?.webSearchConfig?.enabled && (
                                  ...
                                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                      ...
                                      <Input
                                        ...
                                        min="1"
                                        max="10"
                                        value={existingAgent?.webSearchConfig?.maxResults || 5}
                                        onChange={async (e) => {
                                          ...
                                          try {
                                            const response = await fetch(`/api/agent-chatbots/${editAgentId}/web-search-config`, {
                                              ...
                                              headers: { "Content-Type": "application/json" },
                                              body: JSON.stringify({
                                                enabled: existingAgent?.webSearchConfig?.enabled || false,
                                                triggerKeywords: existingAgent?.webSearchConfig?.triggerKeywords || [],
                                                maxResults,
                                                requireWhitelist: existingAgent?.webSearchConfig?.requireWhitelist !== false
                                              }),
                                    ...
                                    <div className="flex items-center justify-between">
                                      ...
                                      <Switch
                                        checked={existingAgent?.webSearchConfig?.requireWhitelist !== false}
                                        onCheckedChange={async (requireWhitelist) => {
                                          try {
                                            const response = await fetch(`/api/agent-chatbots/${editAgentId}/web-search-config`, {
                                              method: "PUT",
                                              headers: { "Content-Type": "application/json" },
                                            ...
                                            console.error("Error updating require whitelist:", error);
                                          }
                                        }}
                                      />
                                    </div>
                            ...
                            <div className="flex items-center gap-2 text-sm">
                              ...
                              <Badge variant={existingAgent?.webSearchConfig?.enabled ? "default" : "secondary"}>
                                {existingAgent?.webSearchConfig?.enabled ? "✅ Enabled" : "❌ Disabled"}
                              </Badge>
                            </div>
                          </CardContent>
                        </Card>

                        {/* Whitelist Management - Only show if web search is enabled */}
                        {existingAgent?.webSearchConfig?.enabled && (
                          <Card>
                            <CardHeader>
                              <CardTitle className="flex items-center gap-2">
                          ...
                          <Card className="border-gray-200 bg-gray-50">
                            <CardContent className="pt-6">
                              <div className="text-center py-8 text-gray-500">
                                ...
                                <p className="text-sm">
                                  Enable web search above to configure whitelisted websites and search settings.
                                </p>
                              </div>
                            </CardContent>
                          </Card>
