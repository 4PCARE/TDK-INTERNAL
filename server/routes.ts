...
app.get("/api/documents/search-consolidated", isAuthenticated, async (req: any, res) => {
  try {
    ...
    const { query, type = "keyword-name-priority", fileName = "true", keyword = "false", meaning = "false" } = req.query;
    ...
    const isFileNameSearch = fileName === "true";
    const isKeywordSearch = keyword === "true";
    const isMeaningSearch = meaning === "true";

    console.log(`Documents page search - User: ${userId}, Query: "${query}", Type: ${type}, FileName: ${isFileNameSearch}, Keyword: ${isKeywordSearch}, Meaning: ${isMeaningSearch}`);

    // Collect all search results (chunks)
    let allChunkResults: any[] = [];

    // Filename search
    if (isFileNameSearch) {
      console.log("Performing filename search...");
      const filenameResults = await storage.searchDocuments(userId, query);
      const filenameSearchResults = filenameResults.map(doc => ({
        id: doc.id.toString(),
        name: doc.name,
        content: doc.content || "",
        summary: doc.summary,
      ...
      allChunkResults.push(...filenameSearchResults);
      console.log(`Filename search returned ${filenameSearchResults.length} results`);
    }
    ...
    if (isMeaningSearch) {
      ...
      const semanticSearchResults = semanticResults.map(result => ({
        ...result,
        searchType: 'semantic'
      }));
      allChunkResults.push(...semanticSearchResults);
      console.log(`Semantic search returned ${semanticSearchResults.length} results`);
    }

    // Keyword search (if needed)
    if (isKeywordSearch && !isMeaningSearch) {
      console.log("Performing keyword search...");
      const keywordResults = await semanticSearchServiceV2.searchDocuments(query, userId, {
        searchType: 'keyword',
        limit: 20
      });
      const keywordSearchResults = keywordResults.map(result => ({
        ...result,
        searchType: 'keyword'
      }));
      allChunkResults.push(...keywordSearchResults);
      console.log(`Keyword search returned ${keywordSearchResults.length} results`);
    }

    console.log(`Total chunk results before consolidation: ${allChunkResults.length}`);

    // Consolidate chunks into documents
    const documentMap = new Map();

    allChunkResults.forEach((result: any) => {
      // Extract original document ID from chunk ID (format: "docId-chunkIndex" or just "docId")
      const originalDocId = result.id.toString().includes('-') 
        ? result.id.toString().split('-')[0] 
        : result.id.toString();

      const docIdNumber = parseInt(originalDocId);

      if (!documentMap.has(docIdNumber)) {
        // First chunk for this document - use it as the main result
        documentMap.set(docIdNumber, {
          id: docIdNumber,
          name: result.name.replace(/ \(Chunk \d+\)$/, ''), // Remove chunk suffix from name
          content: result.content,
          summary: result.summary,
          aiCategory: result.aiCategory,
          aiCategoryColor: result.aiCategoryColor,
        ...
        const existing = documentMap.get(docIdNumber);
        ...
        // Update main document with highest similarity chunk if this one is better
        ...
        // Prefer filename search results over others
        if (result.searchType === 'filename' && existing.searchType !== 'filename') {
          existing.searchType = 'filename';
          existing.similarity = Math.max(existing.similarity, 1.0);
        }
      }
    });

    // Convert map to array and sort appropriately
    const consolidatedResults = Array.from(documentMap.values()).sort((a, b) => {
      // Primary sort: filename results first
      if (a.searchType === 'filename' && b.searchType !== 'filename') return -1;
      if (b.searchType === 'filename' && a.searchType !== 'filename') return 1;
      
      // Secondary sort: by number of chunks (more chunks = higher rank)
      if (a.topChunks.length !== b.topChunks.length) {
        return b.topChunks.length - a.topChunks.length;
      }
      
      // Tertiary sort: by similarity (higher similarity = higher rank)
      return b.similarity - a.similarity;
    });

    console.log(`Final results count after consolidation: ${consolidatedResults.length} documents`);

    // Debug: Log first few consolidated results
    consolidatedResults.slice(0, 3).forEach((doc, index) => {
      console.log(`${index + 1}. Doc ${doc.id}: ${doc.name} (${doc.topChunks.length} chunks, best similarity: ${doc.similarity.toFixed(3)}, type: ${doc.searchType})`);
    });
    ...
    console.error("Documents page search error:", error);
