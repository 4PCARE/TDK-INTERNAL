...
// Chunk-based search method
  async searchDocumentChunks(
    ...
    try {
      ...
      const { documentChunks, documents } = await import('@shared/schema');
      const { eq, inArray, and } = await import('drizzle-orm');

      console.log(`Advanced chunk-based keyword search for: "${query}"`);
      ...
      if (specificDocumentIds && specificDocumentIds.length > 0) {
        chunksQuery = chunksQuery.where(
          and(
            ...
            inArray(documentChunks.documentId, specificDocumentIds)
          )
        );
      }
      ...
      const results = topChunks
        ...
        .map(chunkScore => {
          const chunk = chunks.find(c => c.chunkId === chunkScore.chunkId)!;
      ...
      console.log(`Advanced chunk-based keyword search returned ${results.length} results`);
      return results;

    } catch (error) {
      console.error('Advanced chunk-based keyword search error:', error);
      throw new Error('Advanced chunk-based keyword search failed');
    }
  }

  private calculateChunkScores(chunks: any[], searchTerms: SearchTerm[]): Array<{
    chunkId: number;
    score: number;
    matchedTerms: string[];
    matchDetails: Array<{
    ...
    // Calculate IDF for each search term across chunks
    const termIDF = this.calculateChunkIDF(chunks, searchTerms);
  ...
  private calculateChunkIDF(chunks: any[], searchTerms: SearchTerm[]): Map<string, number> {
    const termIDF = new Map<string, number>();
    ...
    for (const searchTerm of searchTerms) {
      ...
      for (const chunk of chunks) {
        const chunkText = (chunk.content || '').toLowerCase();
        if (this.termExistsInDocument(searchTerm.term, chunkText, searchTerm.fuzzy)) {
          chunksWithTerm++;
        }
      }
  ...
  // AI-Enhanced chunk search with chat history context
  async searchDocumentChunksWithAI(
    query: string,
    userId: string,
    chatHistory: Array<{messageType: string; content: string; createdAt: Date}> = [],
    limit: number = 20,
    ...
    try {
      ...
      console.log(`AI-enhanced chunk-based keyword search for: "${query}"`);
      ...
      const chunks = await chunksQuery;
      console.log(`AI-enhanced chunk search: Found ${chunks.length} chunks to search`);
      ...
      console.log(`Enhanced search terms for chunks:`, enhancedSearchTerms.map(t => `${t.term}(${t.weight}, ${t.source})`));
      ...
      console.log(`AI-enhanced chunk-based keyword search returned ${results.length} results with expansion confidence: ${expansionResult.confidence}`);
      return results;

    } catch (error) {
      console.error('AI-enhanced chunk-based keyword search error:', error);
      // Fallback to regular chunk search
      console.log('Falling back to regular chunk-based keyword search...');
