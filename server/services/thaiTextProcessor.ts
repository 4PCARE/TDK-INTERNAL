...
// Apply bidirectional alias expansion if provided
    if (aliases) {
      console.log(`🔍 Thai: Applying bidirectional alias expansion with ${Object.keys(aliases).length} alias groups`);
      const expandedTerms = new Set(allTerms);

      for (const term of allTerms) {
        const lowerTerm = term.toLowerCase();

        // Method 1: Check if this term is a key in aliases (key -> values)
        if (aliases[term] || aliases[lowerTerm]) {
          const aliasGroup = aliases[term] || aliases[lowerTerm];
          aliasGroup.forEach(alias => {
            expandedTerms.add(alias);
            // Also add segmented version of alias if it's Thai
            if (this.isThaiText(alias)) {
              const segmentedAlias = this.segmentThaiText(alias);
              segmentedAlias.forEach(seg => expandedTerms.add(seg));
            }
          });
          console.log(`🔍 Thai: Expanded key "${term}" with values:`, aliasGroup);
        }
        ...
        for (const [key, values] of Object.entries(aliases)) {
          ...
          if (lowerValues.includes(lowerTerm)) {
            ...
            if (this.isThaiText(key)) {
              const segmentedKey = this.segmentThaiText(key);
              segmentedKey.forEach(seg => expandedTerms.add(seg));
            }
            ...
            console.log(`🔍 Thai: Found "${term}" as value, expanded with key "${key}" and all values:`, values);
      ...
      console.log(`🔍 Thai: Final bidirectionally expanded terms:`, allTerms);
    }